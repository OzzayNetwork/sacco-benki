...
$('#custom_bulk').on('change', function () {
...
function disableBtn() {
	...
	$('.bulk_sms_table input').each(function (value, index) {
		if (this.checked) {
			...
			$('.members_selected').html("0 Members ");
...
function countSelectedMembers() {
	...
	$('.bulk_sms_table input').each(function (value, index) {
		if (this.checked) {
			checkedMembers = checkedMembers + 1;
		}
	});
...
$('.bulk_sms_text').on('keyup', function () {
	...
	$('.bulk_sms_char_count').text(Char + ' Characters');
	if (Char < 160) {
		$('.character_cost').text('KES ' + 1);
		$('.total_cost').text('KES ' + (Cost * checkedMembers));

	} else {
		Cost = Math.round(Char / 160);
		$('.character_cost').text('KES ' + Cost);
		$('.total_cost').text('KES ' + (Cost * checkedMembers));
	}
